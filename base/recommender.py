...
class Recommender(object):
    def __init__(self, conf, training_set, test_set, **kwargs):
        self.config = conf
        self.data = Data(self.config, training_set, test_set)  # 打包config,training_data, test_data
        self.model_name = self.config['model.name'] # e.g. SGL
        self.ranking = OptionConf(self.config['item.ranking']) # getitem {'-topN':'10,20'}
        ...
        self.output = OptionConf(self.config['output.setup']) # getitem {'-dir':'./result'}

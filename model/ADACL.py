...
class ADACL_Encoder(nn.Module):
    def __init__(self, data, emb_size, drop_rate, n_layers, temp, aug_type, reserve_rate):
        super(ADACL_Encoder, self).__init__()
        print(emb_size)
        self.data = data
    ...
    def graph_reconstruction(self, is_reverse):
        dropped_mat = GraphAugmentor.edge_adadrop(self.data.interaction_mat, self.drop_rate, self.reserve_rate, is_reverse)
...
class ADACL(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(ADACL, self).__init__(conf, training_set, test_set)
        ...
        self.model = ADACL_Encoder(self.data, self.emb_size, drop_rate, n_layers, temp, aug_type)
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            model.train()
            dropped_adj1 = model.graph_reconstruction(is_reverse=False)
            dropped_adj2 = model.graph_reconstruction(is_reverse=True)
            for n, batch in enumerate(next_batch_pairwisev2(self.data, self.batch_size)):
                ...
                cl_loss = self.cl_rate * (model.cal_cl_lossv4([user_idx, pos_idx], dropped_adj1, dropped_adj2)) #+ model.cal_cl_lossv4([user_idx, pos_idx], dropped_adj1, dropped_adj2))

...
class SIAB(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        super(SIAB, self).__init__(conf, training_set, test_set)
        args = OptionConf(self.config['SIAB'])
        ...
        self.model = SIAB_Encoder(self.data, self.emb_size, n_layers, self.temp, self.beta, self.maxEpoch, self.aug_type, self.drop_rate)
...
class SIAB_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers, temp, beta, maxepoch, aug_type, drop_rate):
        super(SIAB_Encoder, self).__init__()
        self.temp = temp
    ...
    def calc_lossv3(self, idx, perturbed_mat1, perturbed_mat2):
        ...
        user_mask_tensor = torch.exp(-k*user_mask_tensor) 
        ...
        item_mask_tensor = torch.exp(-k*item_mask_tensor) 
    ...
    def calc_lossv4(self, idx, perturbed_mat1, perturbed_mat2):
        ## lossv4 cpu version

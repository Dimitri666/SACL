...
class MixSimGCL(GraphRecommender):
    def __init__(self, conf, training_set, test_set):
        # training_set,test_set  [[user_id, item_id, float(weight)]...]

        super(MixSimGCL, self).__init__(conf, training_set, test_set)
        ...
        self.model = MixSimGCL_Encoder(self.data, self.emb_size, self.eps, self.n_layers)
...
class MixSimGCL_Encoder(nn.Module):
    def __init__(self, data, emb_size, eps, n_layers):
        super(MixSimGCL_Encoder, self).__init__()
        self.data = data
        self.eps = eps

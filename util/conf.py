...
class OptionConf(object):
    """
    param: str contain conf name and value
    return: dict with name and value
    """
    def __init__(self,content):
        self.line = content.strip().split(' ') # -topN 10,20
        self.options = {}
        ...
        if self.line[0] == 'on':
            ...
            self.mainOption = False
        for i,item in enumerate(self.line): #['-Top', '10,20']
            if (item.startswith('-') or item.startswith('--')) and  not item[1:].isdigit():
                ind = i+1
                for j,sub in enumerate(self.line[ind:]):  #'10,20'
